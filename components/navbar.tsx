    ...
    <header className="sticky top-0 z-50 w-full border-b bg-nixerly-blue text-white backdrop-blur supports-[backdrop-filter]:bg-nixerly-blue/95">
      <div className="container flex h-16 items-center justify-between">
              ...
              <nav className="flex flex-col gap-4 mt-8">
                {navLinks.map((link) => (
                  <Link
                    key={link.href}
                    href={link.href}
                    className={`text-lg font-medium ${isActive(link.href) ? "border-b-2 border-nixerly-blue" : "text-muted-foreground"}`}
          ...
          {navLinks.map((link) => (
            <Link
              ...
              className={`text-base font-medium text-white ${
                isActive(link.href) ? "border-b-2 border-white" : "hover:border-b-2 hover:border-white/70"
              }`}
            >
              {link.label}
            </Link>
        ...
        <div className="hidden md:flex items-center gap-2">
          <Button variant="ghost" className="text-white hover:bg-primary-700" asChild>

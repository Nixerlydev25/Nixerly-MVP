...
import { ONBOARDING_STEPS_WORKER, OnboardingStepWorker } from "../../types/onboarding";

export const useOnboardingNavigation = () => {
  const router = useRouter();
  const searchParams = useSearchParams();
  
  const currentStep = searchParams.get("onboarding-step") as OnboardingStepWorker || OnboardingStepWorker.PERSONAL_INFO;

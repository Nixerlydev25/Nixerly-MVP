export enum QueryKeys {
  AUTH = 'auth',
  SIGN_UP = 'sign-up',
  SIGN_IN = 'sign-in',
  SIGN_OUT = 'sign-out',
  CURRENT_USER = 'current-user',
  USERS = 'users',
  UPDATE_USER = 'update-user',
  GET_USER = 'get-user',
  USER = 'user',
  UPDATE_WORKER_PROFILE = 'update-worker-profile',
  UPDATE_BUSINESS_PROFILE = 'update-business-profile',
  GET_SKILLS = 'get-skills',
  GET_EXPERIENCE = 'get-experience',
  GET_EDUCATIONS = 'educations',
  CREATE_SKILLS = 'create-skills',
  CREATE_LANGUAGES = 'create-languages',
  CREATE_EDUCATIONS = 'create-educations',
  UPDATE_EDUCATIONS = 'update-educations',
  DELETE_EDUCATIONS = 'delete-educations',
  GET_WORKERS = 'get-workers',
  GET_WORKER_PROFILE = 'get-worker-profile',
  GET_WORKER_BY_ID = 'get-worker-by-id',
  LOGOUT = 'logout',
  WORKER_PROFILE_DETAILS = 'worker-profile-details',
  UPDATE_SKILLS = 'update-skills',
  UPDATE_LANGUAGES = 'update-languages',
  JOB_CREATE = 'job-create',
  JOB_GET_ALL = 'job-get-all',
  JOB_DETAILS = 'job-details',
  GET_PROFILE_PICTURE_UPLOAD_URL = 'get-profile-picture-upload-url',
  UPDATE_PROFILE_PICTURE = 'update-profile-picture',
  UPDATE_PROFILE_PICTURE_S3 = 'update-profile-picture-s3',
  GET_BUSINESS = 'get-business',
  BUSINESS_PROFILE_DETAILS = 'business-profile-details',
  APPLY_JOB = 'apply-job',
  GET_JOB_APPLICANTS = 'get-job-applicants',
  REPORT_WORKER = 'report-worker',
  REPORT_BUSINESS = 'report-business',
  REPORT_JOB = 'report-job',
  HAS_BUSINESS_REPORTED_WORKER = 'has-business-reported-worker',
  HAS_WORKER_REPORTED_JOB = 'has-worker-reported-job',
  HAS_WORKER_REPORTED_BUSINESS = 'has-worker-reported-business',
  GET_APPLIED_JOBS = 'get-applied-jobs',
  GET_BUSINESS_PROFILE_PICTURE_UPLOAD_URL = 'GET_BUSINESS_PROFILE_PICTURE_UPLOAD_URL',
  UPDATE_BUSINESS_PROFILE_PICTURE_S3 = 'UPDATE_BUSINESS_PROFILE_PICTURE_S3',
  UPDATE_BUSINESS_PROFILE_PICTURE = 'UPDATE_BUSINESS_PROFILE_PICTURE',
  CURRENT_BUSINESS_PROFILE = 'CURRENT_BUSINESS_PROFILE',
  GET_COMPANY_ASSETS_UPLOAD_URL = 'GET_COMPANY_ASSETS_UPLOAD_URL',
  UPLOAD_COMPANY_ASSETS_TO_S3 = 'UPLOAD_COMPANY_ASSETS_TO_S3',
  DELETE_COMPANY_ASSETS = 'DELETE_COMPANY_ASSETS',
  SAVE_COMPANY_ASSETS = 'SAVE_COMPANY_ASSETS',
  TOGGLE_JOB_STATUS = 'toggle-job-status',
}
